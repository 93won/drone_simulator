...
\begin{itemize}
    ...
    \item CMake 3.20 or higher (required for Micro XRCE-DDS Agent)
\end{itemize}

\textbf{Note:} If your system has CMake 3.16 (Ubuntu 20.04 default), you'll need to upgrade it. Installation instructions are provided in Chapter 2.
...
\begin{itemize}
    \item \textbf{ROS2 Galactic}: A framework for robot software development. This is what I use to write control programs. ROS2 provides tools for creating modular control nodes and managing communication between different parts of the system. \\
    \textit{Reference:} \url{https://docs.ros.org/en/galactic/}
...
ros2 --version
# Should output: ros2 cli version galactic
...
Before installing the Micro XRCE-DDS Agent, you need to ensure CMake 3.20 or higher is installed. Ubuntu 20.04 comes with CMake 3.16 by default, which is not sufficient.

\textbf{Check your CMake version:}
\begin{lstlisting}[language=bash]
cmake --version
# If version is less than 3.20, follow the upgrade steps below
\end{lstlisting}

\textbf{Upgrade CMake to 3.20.6:}
...
The script installs the Micro XRCE-DDS Agent in \texttt{chapter2/agent\_install/}, sets up the ROS2 workspace with PX4 message definitions, and creates helper scripts for convenient usage.
...
\hline
\end{tabular}
\caption{Autonomous Mission Timeline (20Hz control loop)}
\end{table}
